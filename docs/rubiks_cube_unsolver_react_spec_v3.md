...
音声ファイルはプロジェクトルートの `voice/` に配置し、
`{フェーズ番号}_{ファイル番号}.mp3` の命名規則とする。

操作完了後、対象フェーズの設定確率で抽選し、当選した場合はそのフェーズのファイルから均等ランダムに1本再生する。
再生中の音声には別の音声を重ねない。
各フェーズはデモ開始時に独立したシャッフルバッグとしてリセットする。
一巡するまで同じファイルを再利用せず、バッグの再補充時も直前のファイルが続かないようにする。

```text
フェーズ1: 操作1〜5
フェーズ2: 操作6〜完了予定ステップ
フェーズ3: 完了予定ステップ超過後
フェーズ4: 最大操作数到達後のGive Up
```

フェーズ4は確率抽選を行わず、既存音声を停止してからランダムに1本を必ず再生する。
再生完了後に `DEMO_END` へ進む。エラーやリセットによる安全 `STOP` ではフェーズ4を再生しない。
